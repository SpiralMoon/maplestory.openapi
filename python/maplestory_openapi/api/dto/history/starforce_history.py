...
class StarforceHistoryEvent(BaseModel):
    ...
    destroy_decrease_rate(str): 이벤트 파괴 확률 감소율
...
class StarforceHistoryInfo(BaseModel):
    ...
    superior_item_flag(str): 슈페리얼 장비
    destroy_defence(str): 파괴 방지
    chance_time(str): 찬스 타임
    event_field_flag(str): 파괴 방지 필드 이벤트
    upgrade_item(str): 사용 주문서 명
    protect_shield(str): 프로텍트 실드
    bonus_stat_upgrade(str): 보너스 스탯 부여 아이템 여부
    character_name(str): 캐릭터 명
    world_name(str): 월드 명
    target_item(str): 대상 장비 아이템 명
    date_create(str): 강화 일시 (KST)
    starforce_event_list(list[StarforceHistoryEvent]): 진행 중인 스타포스 강화 이벤트 정보
    """
    id: str
...
class StarforceHistory(BaseModel):
    ...
    starforce_history(list[StarforceHistoryInfo]): 스타포스 히스토리
    """
    count: int
